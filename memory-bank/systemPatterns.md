...
### Interaction
Autoload `InteractionManager` (registry + screen-space prompt) and `Interactable` (Area2D: `prompt`, `enabled`, `one_shot`, `auto_pickup`, `interacted` signal). Areas self-register on player contact and unregister on `_exit_tree`; the nearest area wins; nearest is re-resolved only on registry change or keypress (no per-frame scans); player resolves via `Global.player`; freed entries are pruned; prompt prefix derives from the `interact` input binding; `set_locked()` freezes interaction for future modals. Prompt rendering is screen-space: the label lives on the manager's `CanvasLayer` (layer 10, VT323) and the manager re-projects the active area's world anchor through the canvas transform every frame while shown (registry events position synchronously), so bobbing, moving, or rotating owners are followed and world rendering can never occlude the prompt. Anchoring is automatic: the anchor is the top-center of the union of the owner's visible sprite art (alpha-aware used-rect of the displayed frame — frame-relative, cached per texture+frame; rotation/scale/flip/region proof; `CanvasLayer`/`Control` subtrees are excluded from bounds and tree visibility is required); the manager owns the `PROMPT_MARGIN` gap (12 screen px) and sizes the label to its text; `prompt_offset` is a screen-space nudge (default zero); sprite-less areas anchor at the area origin.
...
| Flash palette | On-hit flash WHITE for enemies, RED for the player; stun/vulnerable YELLOW `Color(1, 1, 0, 1)` pulsed on a `FlashPlayer` animation channel (per-animation flash_color tracks own the color); action telegraphs reuse the enemy's own attack strobe channel (necromancer summon/cast share the purple strobe on swapped poses) |
...
| Doors | One script (`entities/interactables/doors/door.gd`) with a `lock_type` export (none/key/boss_key); each scene's "open" animation Call Method track calls `_set_passable()` to drop the blocking collision — no timer constants in code; failed unlocks flash the screen-space prompt "You Need a Key To Open!" for 1s via `InteractionManager.refresh_prompt()` — per-scene Labels are retired; scenes: `key_door.tscn` (locked, `key_door_sheet.png`), `door_red.tscn` (no-lock, eli art, rotate the instance for wall sides), `no_open.tscn` (decorative sealed door); `door.tscn`/`door_2.tscn` kept unused; door/chest blockers sit on Environment; the boss key is a prompted interact pickup that plays its boss's `boss_taunt` DialogueData through the shared dialogue box on first pickup |
| Chests | One script (`entities/interactables/chests/chest.gd`): opens on interact (sprite frame 2) and scatters its `loot_table` roll; `requires_key` consumes a key first with the same prompt-flash feedback as doors. `chest.tscn` = key chest (locked look, red sheet frame 1), `chest_no_key.tscn` = free chest; key-drop behavior retired |
| Props | Scriptless decor scenes in `entities/props/` (`wall_torch`, `side_torch`, `candlestick`, `candlestick_2`): `AnimatedSprite2D` autoplaying `default` at 5.7 fps, radial `PointLight2D`, foot blocker `StaticBody2D` on Environment (layer 4, mask 0). Wall-mounted props render at z 10 (Overhead tier) to overlay wall faces; floor props stay z 0 for y-sort. Side torch is a left-facing profile — flip H / rotate per wall side (door_red convention). Pack per-frame art (`torch_1..4`, `candlestick_1/2_*`) is distinct art from the `objects/` strips (`wall_torch_h`, `torch_animation`) — verified, not interchangeable |
| Dialogue | Per-speaker `DialogueData` .tres (`systems/dialogue/`): portrait (AtlasTexture crop), one-shot stages (`pages` + `requires_flag` gate + `set_flag` on completion), greeting fallback once stages are consumed; consumption lives on the player's `PlayerProgress` (flags + per-speaker stage counters, session-scoped). Author pages as `Array[String]` and flags as `String` — 4.7.2 editor saves strip PackedStringArray/StringName from scripted sub-resources (see techContext). Unified `npc.gd` opens its dialogue on interact and hands off to `shop_data` when the dialogue finishes normally; boss keys play their taunt via the shared box (`boss_key.gd`) |
